...
class Cohort():
    ...
    def add_student(self, dictionary):
        self.students.append(dictionary)
        return None
    
    def list_students(self):
        return self.students
    
    def list_employed_by(self, string):
        employer_list = []
        for student in self.students:
            for value in student.values():
                if value == string:
                    employer_list.append(student)
...
class Person():
    ...
    def get_work_address(self):
        list_address =  self.dictionary["addresses"]
        work_address = list_address[0]
        return f"{work_address['building']} {work_address['street']}"

    def get_home_address(self):
        home = self.dictionary["addresses"][1]
        home_format = f'{home["building"]} {home["street"]}'
        return home_format
    
    def get_pets(self):
        pets_list = self.dictionary["pets"]
        result_str = f"{self.dictionary['name']} has {len(pets_list)} pets:"
        for pet in pets_list:
                if pet == pets_list[-1]:
                    result_str += f" a {pet['animal']} called {pet['name']}"
                else:
                    result_str += f" a {pet['animal']} called {pet['name']},"
